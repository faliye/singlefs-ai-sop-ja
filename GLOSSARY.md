...
| 变异测试 | mutation testing | ミューテーションテスト | 把被测代码改坏，验证断言真的会红 |
| 变异清单 | mutation list | ミューテーションリスト | 入库的「改了哪里 → 哪条断言红」记录，交给复跑门禁反复验证 |
| 等价变异 | equivalent mutant | 等価ミュータント | 在全部输入上与原式同值的变异；永远抓不到，不算盲区 |
| 确定性模型 | deterministic model | 決定的モデル | 无随机源、真实 I/O、并发、时钟输入的被测对象；跑 N 遍必然一致 |
