---
name: gate
description: 本プロジェクトの受入ゲートを走らせる。コードを提出する前、ある変更を受け取れるか判断するときに使う——各段階の意味、結果の判読、どの「失敗」がコードではなく環境の問題かを含む。
---
<!-- generated-from: skills/gate/SKILL.md sha256:c5c2edf8bce8a5f407ac9cd7eb42c076585cda2f19330e6104189dc2260b271c -->

# 受入ゲート

規則は `rules/show-me-test.md`。**ここには走らせ方・判読の仕方・どの失敗が偽物かを書く。**

## 走らせる

```bash
bash .claude/scripts/gate.sh          # 全部。提出前に必ず
bash .claude/scripts/check.sh         # 書式/lint/ビルド/単体テストだけ。速い折り返し
bash .claude/scripts/env.sh           # 環境自己検査だけ
GATE_BASE=<commit> bash .claude/scripts/gate.sh   # diff の基準を指定
bash .claude/scripts/gate.sh --staged # HEAD + ステージ領域だけで走らせる：複数のセッションが一つのリポジトリに同時に書くときに使う。赤になったものが、このコミットが持ち込んだものである
```

## 段階と判読

| 段階 | 失敗が意味すること |
|---|---|
| 規範バージョン | プロジェクトの `.singlefs-ai-sop-version` と singlefs-ai-sop の `VERSION` が食い違う。**まず規則の変更を読み**、それから `install.sh` でスタンプを更新する |
| ゲート自己検査 | どこかの拒否が対処を示していない（`bad` に `howto` が無い、`die` が一引数だけ、直に印字される `✗` の後ろに `→` が無い）、または対象群を走査する検査が成功時に件数を出していない。形態は `rules/sop-first.md` を見る |
| ゲート判別力 | 標本の判定が予期と違う——**ゲート自身が壊れている**。他より先にそれを直す |
| shell 規律 | スクリプトに `pkill -f` / `killall` / `pgrep -f` がある、サブシェル内の代入で値を外へ持ち出している、git の取り消し系コマンドがある、または `rm -rf` を守りのない変数パスに当てている。`rules/command-safety.md` を見る |
| 文書鉄則 | 本文に履歴記述が混ざっている、kb の番号が簡称なしで引用されている、または CLAUDE.md が規則を一条ずつ `@` していない。`rules/writing-discipline.md` を見る |
| 命名規律 | `.rs` で自前に宣言した名前が一文字かよくある略語である、または `.claude/abbreviations`・`.claude/naming-lint-exclude` の書き方が不正。`rules/code-discipline.md` を見る |
| Show me test | `crates/*/src` を変えたのにテストが無い。**ここは迂回不可**。`rules/show-me-test.md` を見る |
| ビルドと単体テスト | 本当に壊れているか、cargo が無い。clippy は `-D warnings` で判定し、閉じた集合の列挙型に `_ =>` を書くことも拒む |
| プロジェクト固有段階 | `.claude/gate.d/` のローカル検査が赤、または読めない。「覆盖声明（…）」（覆いの宣言）が赤なら、`# gate-covers:` に一覧に無い項目が書かれている |
| 実行中に作業ツリーが変わらなかったか | ゲートが走っている間に作業ツリーのファイルが変わった（自分がまだ編集している、または別のセッションが編集している）ので、各段階が読んだものは同じ版ではない。編集が止まってから走らせ直すか、`--staged` を使う |
| 走り終えて一時ファイルを残していない | どこかの段階（またはそれが起こしたテストや装置）がこの実行の `TMPDIR` に何かを作り、走り終えても消していない。名前と大きさはその節に並ぶ。作った側が走り終えたときに自分で消すようにする。実行をまたいで再利用するつもりのキャッシュは `${GATE_CROSS_RUN_TMPDIR:-${TMPDIR:-/tmp}}` の下に置く。ほかの段階が赤のときはこの項目を今回判定せずと記録し、一時ディレクトリを丸ごと残して現場を見られるようにする（最新の 3 つだけ残し、それより古いものは後で走った実行が消す）。`rules/command-safety.md`「テストイメージは必ず一時ディレクトリに置く」を見る |

**SOP リポジトリ自身でしか走らない段階が三つ**（利用側プロジェクトからは見えない）：
各言語同期、バージョン規律、CHANGELOG 連続性。

「規則マニフェスト」は両側で走るが、問う内容が違う。SOP リポジトリ内では
「マニフェストが規則と歩調を合わせているか」を問い、プロジェクト内では
**入れた副本そのもの**を突き合わせる——副本が改変されたり不完全だったりすれば赤になる。
入れているのが en / ja の副本なら、この項目は「不适用」（該当せず）と報じる：マニフェストは参照リポジトリ（zh）だけで維持され、
訳本リポジトリが追随しているかは zh リポジトリの「各言語同期」が判定する。

## 未実装の段階

`gate.sh` は毎回、共有ゲートが**実装していない**検証手段を並べる：モデル対照テスト、クラッシュ点リプレイ、最終判定基準、
それに shell スクリプトの命名規律。前の三つはプロジェクトが `.claude/gate.d/` でつなぐしかない：モデル対照テストは被検対象の理想モデルを、クラッシュ点リプレイは被検対象自身の書き込み記録と checker を要し、
最終判定基準はプロジェクトが決める。
つないだ段階は冒頭に `# gate-covers: <その項目>` と書き（キーは `gate.sh` の綴りのまま）、
その段階がこの回に走って通ったときだけ、その項目は「プロジェクト固有段階が覆う」の下へ移る。

**これは雑音ではなく、結果を判読する前提である**：ゲートが全緑でも言えるのは「文書が規則どおり＋テストがある＋
単体テストが通る」だけで、それに「プロジェクト固有段階が覆う」の欄に並んだ段階がそれぞれ検証した範囲が加わるにすぎない。
クラッシュ点リプレイを覆う段階が無いうちは、「書き込み経路は検証済み」という言い分はすべて偽である。
覆う段階があっても、その段階が列挙した書き込み経路までしか言えない。

## よくある偽の失敗

| 現象 | 本当の原因 |
|---|---|
| Show me test が「判定対象なし」と言う | 作業ツリーが基準と同じ。**通過でも失敗でもない**。変更してから再実行するか `GATE_BASE=<ref>` で基準を指定する |
| 「上流より古いかは未確認」 | 上流リポジトリが兄弟ディレクトリに無い。この項目は**確認できなかった**もので、集約に別立てで並ぶ——通過と読まないこと |
| 段階をつないだのに、未実装一覧にまだその項目が並ぶ | その段階の冒頭に `# gate-covers:` が無いか、この回は 77 で終わったか赤になった。走って通った段階だけが項目を移す |
| ビルド段階が cargo 不在と言う | 環境の問題。`env.sh` で全体を見て、ツールチェーンを入れて再実行 |
| doc-lint が規則文書の中で挙げた例を指摘する | そのファイルに `<!-- doc-lint:rule-definition -->` が無いか、例がバッククォート／「」の中に置かれていない。印があっても本文はそのまま見られ、免除されるのはこの二つの書き方の中の例だけである |
| Show me test がテスト無しと言うが確かに書いた | テストが `crates/*/src/` の中にあって `#[cfg(test)]`/`#[test]` が付いておらず、スクリプトから見えない |

## ゲート自身も失敗できなければならない

`gate.sh` や `doc-lint.sh` を変えたら、**止められるべき入力を作って本当に赤くなるか確かめる**：

```bash
# 拒否されるべき標本を作り、doc-lint に食わせて赤くなることを確かめる
d=$(mktemp -d); mkdir -p "$d/kb"
printf '# 決定\n\nノードサイズは 16K（以前は 4K）。\n\n## 改訂履歴\n\n### %s\n- 作成。\n' "$(date +%F)" \
  > "$d/kb/decisions.md"
bash .claude/singlefs-ai-sop/scripts/doc-lint.sh "$d"; echo "終了コード $? —— 1 のはず"
rm -rf "$d"
```

⚠️ **標本は別のディレクトリに作ること。本物の kb ファイルの末尾に `>>` してはいけない。**
追記した内容は「改訂履歴」見出しより後ろに落ち、本文走査はそこで既に止まっている——
終了コードは 0 になり、「検査が何もしていない」ように見えるが、実際は標本を作る場所が
間違っているだけである（監査でこの skill 自身の例に対して実測された）。

検査を変えたら `bash .claude/singlefs-ai-sop/scripts/selftest.sh` も走らせる。
`scripts/fixtures/` の標本を使って、各検査がまだ赤くなれることを証明する。
**検査を一つ足したら標本も一つ足す。** その `want=` にはその検査自身のメッセージを書く——
複数の検査が共有する断片では何も見張れない（`rules/show-me-test.md`）。

`rules/show-me-test.md` に従い、赤くなることを示せない検査は書いていないのと同じである。
