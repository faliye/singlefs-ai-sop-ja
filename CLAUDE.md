...
**master のプッシュは `bash scripts/push-all.sh` だけを通し、すべての言語リポジトリを一緒に検証して一緒にプッシュする。**
まずすべての言語リポジトリを検証し（master 上、作業ツリーがクリーン、VERSION が同じ、ゲートが緑）、検証が終わってから
リモートに接続し、検証したそのコミットを一つずつプッシュする。検証のあとでどれかのリポジトリの HEAD が変わっていれば、一つもプッシュしない。
各言語リポジトリで一度ずつ `git config core.hooksPath scripts/githooks` を実行する：フックは master への直接の `git push` を
その場で拒み、push-all.sh を案内する。master 以外の ref（フィーチャーブランチ、タグ）はフックがそのまま通し、指定した ref だけがプッシュされる。
